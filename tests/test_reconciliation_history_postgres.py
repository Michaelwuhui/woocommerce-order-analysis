...
def test_history_postgres_uses_real_schema_and_does_not_replay_stock(pg):
    ...
    items=json.loads(c.execute("SELECT line_items FROM orders WHERE id='1-history'").fetchone()['line_items'])
    items[0]['name']='Fumot RandM Tornado 9000 Puffs - Mint'
    c.execute("UPDATE orders SET line_items=? WHERE id='1-history'",(json.dumps(items),))
    c.execute("INSERT INTO brands(id,name,aliases) VALUES (999,'FUMOT','[]')")
    c.execute("INSERT INTO product_costs(brand_id,puff_count,warehouse_id,cost_price,cost_currency,effective_date,country) VALUES (999,9000,1,39,'CNY','2026-07-01','PL')")
    ...
    assert snap['counts']['cost_matched_quantity']=='2'
    assert snap['totals']['supplier_goods_value_cny']=='78.00'
    assert snap['totals']['product_profit']=='17.14'
    assert snap['totals']['contribution_profit']=='6.15'
