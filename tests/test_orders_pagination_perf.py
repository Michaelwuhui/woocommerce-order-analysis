...
if os.environ.get("WOO_ANALYSIS_RUN_LIVE_PERF_TESTS") != "1":
    pytest.skip(
        "requires an explicit live database performance-test opt-in",
        allow_module_level=True,
    )

ROOT = Path(
    os.environ.get("WOO_ANALYSIS_LIVE_ROOT")
    or Path(__file__).resolve().parents[1]
).resolve()
DB_PATH = ROOT / "woocommerce_orders.db"
if not DB_PATH.is_file():
    pytest.skip(f"live database not found: {DB_PATH}", allow_module_level=True)
