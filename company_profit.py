...
def init_company_profit_tables(get_db_connection):
    """Create the isolated finance tables and strict per-user permission flags."""
    conn = get_db_connection()
    try:
        user_columns = _table_columns(conn, "users")
        added_permission = False
        if "can_view_company_profit" not in user_columns:
            conn.execute(
                "ALTER TABLE users ADD COLUMN can_view_company_profit INTEGER DEFAULT 0"
            )
            added_permission = True
        if "can_edit_company_profit" not in user_columns:
            conn.execute(
                "ALTER TABLE users ADD COLUMN can_edit_company_profit INTEGER DEFAULT 0"
            )
            added_permission = True

        # Initial owner grant is only applied during the first migration.  Later
        # user-management changes are never reset on application restart.
        if added_permission:
            conn.execute(
                """
                UPDATE users
                SET can_view_company_profit = 1,
                    can_edit_company_profit = 1
...
def _permission_flags(get_db_connection, user_id):
    conn = get_db_connection()
    try:
        row = conn.execute(
            """
        ...
        return {
            "view": bool(row and row["can_view_company_profit"]),
            "edit": bool(row and row["can_edit_company_profit"]),
        }
...
def create_company_profit_blueprint(
    get_db_connection,
    compute_sales_board,
    revenue_status_cond,
    partner_recon_detail=None,
    ...
    bp = Blueprint("company_profit", __name__)

    def require_view(function):
        @wraps(function)
        def wrapper(*args, **kwargs):
            flags = _permission_flags(get_db_connection, current_user.id)
            if not flags["view"]:
                if request.path.startswith("/api/"):
                    return jsonify({"error": "无公司盈利查看权限"}), 403
                return (
                    render_template(
                        "company_profit_denied.html",
                        permission_name="公司盈利 · 查看",
                    ),
                    403,
                )
    ...
    def require_edit(function):
        @wraps(function)
        def wrapper(*args, **kwargs):
            flags = _permission_flags(get_db_connection, current_user.id)
            if not flags["edit"]:
                return jsonify({"error": "无公司盈利修改权限"}), 403
            return function(*args, **kwargs)

        return wrapper

    @bp.app_context_processor
    def inject_company_profit_permissions():
        if not current_user.is_authenticated:
            return {
                "can_view_company_profit": False,
                "can_edit_company_profit": False,
            }
        flags = _permission_flags(get_db_connection, current_user.id)
        return {
            "can_view_company_profit": flags["view"],
            "can_edit_company_profit": flags["edit"],
        }

    @bp.route("/company-profit")
    @login_required
    @require_view
    def company_profit_page():
        ...
        try:
            summary = build_company_profit_summary(
                ...
                revenue_status_cond,
                selected_month,
                partner_recon_detail=partner_recon_detail,
                statement_split=statement_split,
                prefer_reconciled_snapshots=prefer_reconciled_snapshots,
            )
    ...
    @bp.route("/api/company-profit/summary")
    @login_required
    @require_view
    def company_profit_summary_api():
        try:
            summary = build_company_profit_summary(
                get_db_connection,
                compute_sales_board,
    ...
    @bp.route("/api/company-profit/settings", methods=["POST"])
    @login_required
    @require_edit
    def save_company_profit_settings():
        payload = request.get_json(silent=True) or {}
        try:
            year_month = _valid_month(payload.get("year_month"))
            payroll_actual = _optional_amount(
                payload.get("payroll_actual_override"), "实际工资"
            )
            payroll_forecast = _optional_amount(
                payload.get("payroll_forecast_override"), "预测工资"
            )
            notes = (payload.get("notes") or "").strip()[:1000]
            calculation_mode = (
                payload.get("calculation_mode") or "percentage"
            ).strip()
            if calculation_mode not in CALCULATION_MODES:
                raise ValueError("核算口径必须是按分润比例或按实际成本")
            actual_expenses_complete = (
                1 if payload.get("actual_expenses_complete") else 0
            )
        except ValueError as exc:
            return jsonify({"error": str(exc)}), 400
        ...
        try:
            conn.execute(
                ...
                INSERT INTO company_profit_month_settings (
                    year_month, calculation_mode, payroll_actual_override,
                    payroll_forecast_override, actual_expenses_complete,
                    notes, updated_by, updated_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
                ON CONFLICT(year_month) DO UPDATE SET
                    calculation_mode = excluded.calculation_mode,
                    payroll_actual_override = excluded.payroll_actual_override,
                    payroll_forecast_override = excluded.payroll_forecast_override,
                    actual_expenses_complete = excluded.actual_expenses_complete,
    ...
    @bp.route("/api/company-profit/market-rules", methods=["POST"])
    @login_required
    @require_edit
    def save_company_profit_market_rules():
        payload = request.get_json(silent=True) or {}
        try:
            year_month = _valid_month(payload.get("year_month"))
        except ValueError as exc:
        ...
        if not isinstance(raw_rules, list):
            return jsonify({"error": "市场口径必须是列表"}), 400

        normalized = []
        try:
            for rule in raw_rules:
                country = (rule.get("country") or "").strip().upper()[:10]
                if not country:
                    raise ValueError("市场代码不能为空")
                raw_share = rule.get("share_rate")
                share_rate = None
                if raw_share not in (None, ""):
                    share_rate = float(raw_share)
                    if not math.isfinite(share_rate) or not 0 <= share_rate <= 1:
                        raise ValueError(f"{country} 分润比例必须在 0% 到 100% 之间")
                forecast_net = _optional_amount(
                    rule.get("forecast_net_sales_cny"),
                    f"{country} 预测净销售",
                )
                notes = (rule.get("notes") or "").strip()[:500]
                normalized.append(
                    (country, share_rate, forecast_net, notes)
                )
        ...
        conn = get_db_connection()
        try:
            for country, share_rate, forecast_net, notes in normalized:
                conn.execute(
                    """
                    INSERT INTO company_profit_market_rules (
                        year_month, country, share_rate,
                        forecast_net_sales_cny, notes, updated_by, updated_at
                    ) VALUES (?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
                    ON CONFLICT(year_month, country) DO UPDATE SET
                        share_rate = excluded.share_rate,
                        forecast_net_sales_cny = excluded.forecast_net_sales_cny,
                        notes = excluded.notes,
                        updated_by = excluded.updated_by,
                        updated_at = CURRENT_TIMESTAMP
                    """,
    ...
    @bp.route("/api/company-profit/forecast-scenario", methods=["POST"])
    @login_required
    @require_edit
    def save_company_profit_forecast_scenario():
        payload = request.get_json(silent=True) or {}
        try:
            year_month = _valid_month(payload.get("year_month"))
            target_net_sales = _required_amount(
                payload.get("target_net_sales_cny"),
                "目标结算净销售额",
            )
            company_revenue_rate = float(
                payload.get("company_revenue_rate")
            )
            if (
                not math.isfinite(company_revenue_rate)
                or not 0 <= company_revenue_rate <= 1
            ):
                raise ValueError("公司收入率必须在 0% 到 100% 之间")
            payroll = _required_amount(payload.get("payroll_cny"), "工资成本")
            fixed_cost = _required_amount(
                payload.get("fixed_cost_cny"),
                "固定经营成本",
            )
            variable_cost = _required_amount(
                payload.get("variable_cost_cny"),
                "其他可变成本",
            )
            notes = (payload.get("notes") or "").strip()[:1000]
        except (TypeError, ValueError) as exc:
        ...
        try:
            conn.execute(
                ...
                ON CONFLICT(year_month) DO UPDATE SET
                    target_net_sales_cny = excluded.target_net_sales_cny,
                    company_revenue_rate = excluded.company_revenue_rate,
                    payroll_cny = excluded.payroll_cny,
                    fixed_cost_cny = excluded.fixed_cost_cny,
                    variable_cost_cny = excluded.variable_cost_cny,
                    notes = excluded.notes,
                    updated_by = excluded.updated_by,
                    updated_at = CURRENT_TIMESTAMP
                """,
    ...
    def normalize_expense_payload(payload):
        ...
        if scenario not in EXPENSE_SCENARIOS:
            raise ValueError("支出类型必须是实际或预测")
        category = (payload.get("category") or "").strip()[:50]
        name = (payload.get("name") or "").strip()[:120]
        if not category or not name:
            raise ValueError("支出分类和名称不能为空")
        amount = _required_amount(payload.get("amount_cny"), "支出金额")
        notes = (payload.get("notes") or "").strip()[:500]
        recurring = 1 if payload.get("is_recurring") else 0
        return year_month, scenario, category, name, amount, recurring, notes

    @bp.route("/api/company-profit/expenses", methods=["POST"])
    @login_required
    @require_edit
    def add_company_profit_expense():
        try:
            values = normalize_expense_payload(
    ...
    @bp.route("/api/company-profit/expenses/<int:expense_id>", methods=["PUT"])
    @login_required
    @require_edit
    def update_company_profit_expense(expense_id):
        try:
            values = normalize_expense_payload(
                request.get_json(silent=True) or {}
            )
        ...
        try:
            cursor = conn.execute(
                """
                UPDATE company_profit_expenses
                SET year_month = ?, scenario = ?, category = ?, name = ?,
                    amount_cny = ?, is_recurring = ?, notes = ?,
                    updated_at = CURRENT_TIMESTAMP
                WHERE id = ?
                """,
                (*values, expense_id),
            ...
            if cursor.rowcount == 0:
                return jsonify({"error": "支出记录不存在"}), 404
            conn.commit()
            return jsonify({"success": True})
        finally:
            conn.close()
    ...
    @bp.route(
        "/api/company-profit/expenses/<int:expense_id>",
        methods=["DELETE"],
    )
    @login_required
    @require_edit
    def delete_company_profit_expense(expense_id):
        conn = get_db_connection()
        try:
            cursor = conn.execute(
                "DELETE FROM company_profit_expenses WHERE id = ?",
                (expense_id,),
            )
            if cursor.rowcount == 0:
                return jsonify({"error": "支出记录不存在"}), 404
