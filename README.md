...
- WooCommerce 库存按站点独立自动同步：关闭/观察/正式/自动暂停、共享库存权重配额、写后回读、批量归零保护和完整运行审计。
...
依次检查全局自动任务开关、站点模式、下一次运行时间和 `inv_push_runs`。观察模式只记录演练；正式模式还会检查 SKU 数量数据源、共享库存站点是否成组启用、外部 WMS 数据新鲜度和异常大幅归零。连续失败达到阈值后站点会自动暂停，必须先修复原因再人工恢复。
...
- 增加每站独立的 WooCommerce 库存自动同步开关，新站和全局开关默认关闭。
- 增加观察模式、正式模式、自动暂停、站点锁、运行审计和配置变更审计。
- 同一数量库存池按站点权重分配，阻止把一份有限库存完整复制到多个站点。
- 正式写入前检查映射/库存数据源，写入后 GET 回读；相同值不重复 PUT。
- 增加批量归零/大幅下降拦截、外部库存过期拦截和连续失败熔断。
- 权限按“全局管理员、库存管理员、站点负责人、库存查看者、仓库合作方”分层。
