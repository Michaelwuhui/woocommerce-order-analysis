...
模块化货权、管理费、利润与资金核销：参见 [合伙人对账 V2 使用与部署](docs/partner-reconciliation-v2.md)。新模块须显式迁移和启用，旧账单保持原入口。
...
| 系统设置 → 站点与同步 | 快速、定时、深度及清理同步；持久化任务、进度、取消、恢复和日志；按站点检查 API 读取能力 |
...
销售看板可按月份、币种填写实际到账后的结算汇率，并将其用于提成及新导出的 Excel；使用方法和 PostgreSQL 迁移见[实际结算汇率说明](docs/sales-board-settlement-rate.md)。
...
发货请求超时或返回不明确时，系统保留操作记录及待核对状态。后台按原站点、订单、运单、承运商、商品身份和数量读取 WooCommerce 结果；证据一致后，原子补齐本地记录。VillaTheme / 自定义商品行接口在两次核验均无运单且满足原包裹恢复条件时，自动补写原运单并再次回读确认，最多尝试三次。恢复保留原发货时间，不新增包裹，不调用邮件或库存接口。

Celery 每 60 秒扫描一次，首次核对至少等待 5 分钟；后续按退避间隔重查。“核对同步”可跳过较长的退避，但保留 5 分钟等待。证据不足、退件、补发、部分发货等冲突场景保留“需处理”，不会把未知结果当作成功。AST 等追加包裹接口不自动重放；被明确拒绝的发货保留单独的原包裹人工重试流程。
...
- PostgreSQL 清理同步由管理员从设置页手动启动，或显式开启每周 Celery Beat 计划；计划默认关闭。逐站完整读取 WooCommerce 订单 ID，对候选逐单核验详情 404 和垃圾箱，遇到不完整分页、异常大量候选或读取错误即停止该站点。确认删除且无其他业务关联的订单，连同备注先归档，再从当前订单表移出；每站归档与移出在同一事务中完成。旧 SQLite 模式仍使用原入口实现。
